...
const hairCap = new THREE.SphereGeometry(0.5, 22, 12, 0, Math.PI * 2, 0, Math.PI * 0.55);
const lensRing = new THREE.TorusGeometry(0.04, 0.006, 6, 18);
...
export function buildHumanoid(look: Appearance): HumanoidRig {
  ...
  add(hips, part(GEOMETRY.sphere, trousers, [0.36 * width, 0.2, 0.25], [0, -0.02, 0]));
  const leg = (side: 1 | -1): Limb => {
    const hip = joint(hips, 0.1 * side * width, -0.03, 0);
    add(hip, part(GEOMETRY.capsule, trousers, [0.16, THIGH / 2, 0.16], [0, -THIGH / 2, 0]));
    const knee = joint(hip, 0, -THIGH, 0);
    add(knee, part(GEOMETRY.capsule, trousers, [0.13, SHIN / 2, 0.13], [0, -SHIN / 2, 0]));
    const foot = joint(knee, 0, -SHIN, 0);
    add(
      foot,
      part(GEOMETRY.sphere, mat(look.shoes, { roughness: 0.6 }), [0.11, 0.075, 0.24], [0, -0.02, 0.05])
    );
  ...
  if (look.jacket) {
    add(spine, part(GEOMETRY.capsule, outer, [0.43 * width, 0.235, 0.27], [0, 0.24, 0]));
    // Open front shows the shirt underneath.
    add(spine, part(GEOMETRY.box, shirt, [0.11 * width, 0.28, 0.02], [0, 0.28, 0.13]));
  } else {
    add(spine, part(GEOMETRY.capsule, shirt, [0.41 * width, 0.23, 0.255], [0, 0.24, 0]));
  }
  add(
    spine,
    part(GEOMETRY.box, mat(look.accent, { roughness: 0.5 }), [0.06, 0.04, 0.012], [0.1 * width, 0.33, 0.138]),
    false
  );

  // Neck and head
  const neck = joint(spine, 0, 0.47, 0);
  add(neck, part(GEOMETRY.cylinder, skin, [0.11, 0.07, 0.11], [0, 0.02, 0]));
  const head = joint(neck, 0, 0.05, 0);
  add(head, part(GEOMETRY.sphere, skin, [HEAD.w, HEAD.h, HEAD.d], [0, HEAD.y, 0]));
  const faceZ = HEAD.d / 2 - 0.012;
  for (const side of [1, -1]) {
    add(head, part(GEOMETRY.sphere, skin, [0.05, 0.08, 0.05], [0.148 * side, HEAD.y, 0]));
    head.add(
      part(
        GEOMETRY.sphere,
  ...
  if (look.glasses)
    head.add(part(GEOMETRY.box, mat('#2b2f36'), [0.04, 0.008, 0.008], [0, HEAD.y + 0.03, faceZ + 0.014]));
  add(head, part(GEOMETRY.sphere, skin, [0.045, 0.055, 0.05], [0, HEAD.y - 0.015, faceZ + 0.008]), false);
  ...
  for (const side of [-1, 1]) {
    head.add(
      part(
        ...
        mat('#ffffff'),
        [0.009, 0.011, 0.006],
        [0.055 * side, HEAD.y + 0.033, faceZ + 0.014]
      )
    );
    head.add(
      part(
  ...
  head.add(part(GEOMETRY.sphere, mat('#a36d5b'), [0.055, 0.012, 0.01], [0, HEAD.y - 0.065, faceZ - 0.002]));
  if (look.jacket)
    for (const side of [-1, 1])
      spine.add(
        part(GEOMETRY.box, shirt, [0.065, 0.13, 0.014], [side * 0.058, 0.38, 0.137], [0, 0, side * 0.3])
      );
  ...
  const arm = (side: 1 | -1): Limb => {
    ...
    add(shoulder, part(GEOMETRY.capsule, outer, [0.115, UPPER_ARM / 2, 0.115], [0, -UPPER_ARM / 2, 0]));
    const elbow = joint(shoulder, 0, -UPPER_ARM, 0);
    add(elbow, part(GEOMETRY.capsule, outer, [0.1, FOREARM / 2 - 0.01, 0.1], [0, -FOREARM / 2 + 0.01, 0]));
    const hand = joint(elbow, 0, -FOREARM, 0);
    add(hand, part(GEOMETRY.sphere, skin, [0.095, 0.11, 0.08], [0, 0, 0]));
  ...
  return { root, scale, hips, spine, head, armL, armR, legL, legR, outlinable };
}
...
function buildHair(
  ...
  const cap = (grow: number) =>
    add(
      head,
      part(hairCap, hair, [HEAD.w * grow, HEAD.h * grow, HEAD.d * grow], [0, y + 0.005, -0.008], [-0.3, 0, 0])
    );
  switch (look.hairStyle) {
    case 'short':
      cap(1.08);
      add(head, part(GEOMETRY.sphere, hair, [0.28, 0.2, 0.15], [0, y - 0.03, -0.095]));
      return;
    case 'bob':
      cap(1.1);
      add(head, part(GEOMETRY.sphere, hair, [0.35, 0.31, 0.3], [0, y - 0.07, -0.05]));
      return;
    case 'bun':
      cap(1.08);
      add(head, part(GEOMETRY.sphere, hair, [0.15, 0.14, 0.15], [0, y + 0.17, -0.12]));
      return;
    case 'ponytail':
      cap(1.09);
      add(head, part(GEOMETRY.sphere, hair, [0.09, 0.09, 0.09], [0, y + 0.06, -0.165]));
      add(head, part(GEOMETRY.capsule, hair, [0.09, 0.12, 0.085], [0, y - 0.1, -0.2], [0.3, 0, 0]));
      return;
    case 'curly':
      cap(1.2);
      for (let i = 0; i < 8; i++) {
      ...
      add(head, part(GEOMETRY.sphere, hair, [0.34, 0.28, 0.25], [0, y - 0.04, -0.09]));
      return;
    case 'wavy':
      cap(1.09);
      add(head, part(GEOMETRY.sphere, hair, [0.18, 0.09, 0.13], [0.05, y + 0.15, 0.085], [0, 0, -0.3]));
      add(head, part(GEOMETRY.sphere, hair, [0.28, 0.2, 0.15], [0, y - 0.03, -0.095]));
  }
}
