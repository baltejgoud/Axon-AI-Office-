...
function bookshelf(itemDef: FurnitureItem): THREE.Group {
  const { w, d } = itemDef;
  const height = 2.2;
  ...
  for (let bay = 0; bay < bays; bay++) {
    for (let s = 0; s < shelves; s++) {
      ...
      let x = -w / 2 + bay * bayWidth + 0.05;
      const end = x + bayWidth - 0.1;
      if (random() < 0.18) {
      ...
      while (x < end - 0.04) {
        ...
        g.add(
          part(
            ...
            [width, bookHeight, d * 0.7],
            [x + width / 2, shelfY + bookHeight / 2, 0.02],
            [0, 0, lean]
          )
        );
