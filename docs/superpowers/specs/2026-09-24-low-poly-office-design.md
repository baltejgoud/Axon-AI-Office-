...
1. **The Lounge turns to face the TV.** The fixed camera looks from the front, so a TV at the U's old open (south) end, facing the main sofa, would have shown only its back and the game would never be seen. The media wall (TV over a walnut console with a PS5, soundbar, record player and two controllers) goes on the back wall facing the viewer; the main sofa moves to the south end facing it; the side sofas move 0.6 m north; the bean bags sit in front of the screen as the two gaming seats. The TV is still at the U's open end, facing the main sofa. The art moves either side of the TV; the three Lounge sofas share one fabric.
2. **Gaming break** (`gaming`, 40–90 s): coworkers near the Commons take a bean bag and a controller; a coworker quietly working at their desk near the Commons joins on the other bag when the away budget has room for both. It shares the 12-away cap and seat reservations, never starts on a task, and a task pulls a player straight back (the controller stays with the game). The TV plays a kart race while someone plays and a screensaver otherwise, redrawn only while the Lounge is close and on screen.
3. **Play corners.** The decor planner packs each strip along its long side, spreads the wishes evenly and shares them across a department's strips, then adds one tree. New groupings: games (foosball, arcade, dartboard), snacks (vending machine, snack shelf) and a TV corner. Every Engineering department gets a games corner and snacks or a TV corner, and no stretch of bare floor wider than 4 m (tested); the AI lab spreads racks, data wall, games, snacks, TV corner and a lounge over its four strips.
4. **Play break** (`play`, 30–70 s): foosball for two from one department (a settled teammate joins), or the arcade alone; only at the department's own corner (tested over twenty minutes of the whole campus).
5. **Foosball rods** for every table are two instanced meshes (one per team): two draw calls for the campus, turning only while the table is in use.
6. **Triangles, measured per kind:** bean bags drop to mid-poly spheres (100k → 30k), coffee tables, the TV corners and the media console carry a light potted tuft instead of a full bush (coffee tables 40k → 14k), fruit uses the coarse facet.
7. **The office dog** is one baked faceted mesh on a cushion; it breathes (a slow swell), still with reduced motion.
8. **Desktop check.** The café, Lounge and games-corner steps run after the draw-call budget reading (they send people on breaks, and people away are drawn in full); the whole check may take up to 240 s. The power went off during the Part 4 runs, so the check now asserts the 1080p floor only on mains power, as agreed, and reports the reading on battery.
9. **Readings:** whole campus 412 draw calls, 1.60M triangles. On battery: 1080p Balanced 51 fps (median of ten), High 34 fps. An A/B against the Part 3 build at the same power showed no difference (late-state 55–59 fps both).
